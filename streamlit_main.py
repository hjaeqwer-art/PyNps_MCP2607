...
class PensionData():
    ...
    def find_company(self, company_name):
        return self.df.loc[self.df['사업장명'].str.contains(company_name), ['사업장명', '월급여추정', '연간급여추정', '업종코드', '가입자수']].sort_values('가입자수', ascending=False)
...
if data and company_name:
    ...
    if len(output) > 0:
        ...
        # plt.pyplot(1, 2) -> plt.subplots(1, 2) 로 수정
